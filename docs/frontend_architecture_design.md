...
5. **前后端完全解耦与独立部署**：配置 Vite 开发代理，生产环境独立构建并配合 Nginx 或 CDN 部署。
...
### 8.2 生产构建与独立部署
- **生产构建**：执行 `npm run build`，编译产物输出至 `frontend/dist/`。
- **独立部署**：配合 Nginx 反向代理托管前端静态文件，并将 `/api/` 及 `/v1/` 代理转发至 FastAPI 后端集群。
